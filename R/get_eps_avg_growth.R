...
#'
get_eps_avg_growth <- function(df) {
  check_sanity(df)
  ...
  for(i in 1:rows) {
    ...
    ticker <- df$Symbol[i]
    url <- paste0("https://www.gurufocus.com/term/growth_per_share_eps/", ticker, "/")

    xpath <- '//*[@id="target_def_historical_data"]/div[2]/div/table'
    table <- get_html_table(url, xpath = xpath, name = paste0(pfix, "_20"))
    ...
    colnames(dfc) <- paste0(pfix, "_ttm")

    pe_table <- cbind(ticker, table, dfc)
    colnames(pe_table)[1] <- "Symbol"

    if (check_table_topicality(table, ttmpe)) {
      pe_table <- data.frame("Symbol" = ticker, "ttm" = NA)
      names(pe_table)[2] <- paste0(pfix, "_ttm")
    }

    dfe <- plyr::rbind.fill(dfe, pe_table)
  }

  df <- merge_with_input(new_data = dfe, input = df, kpi = pfix)
  return(df)
}

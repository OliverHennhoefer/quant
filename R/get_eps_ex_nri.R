...
get_eps_ex_nri <- function(df) {
  ...
  for(i in 1:rows) {
    ...
    ticker <- df$Symbol[i]
    url <- paste0("https://www.gurufocus.com/term/eps_nri/", ticker, "/")

    xpath <- '//*[@id="target_def_historical_data"]/div[2]/div/table'
    table <- get_html_table(url, xpath = xpath, name = "eps_ex_nri_20")
    ...
    colnames(dfc) <- "eps_ex_nri_TTM"

    pe_table <- cbind(ticker, table, dfc)
    colnames(pe_table)[1] <- "Symbol"

    if (check_table_topicality(table, ttmpe))
    { pe_table <- data.frame("Symbol" = ticker, "eps_ex_nri_TTM" = NA) }

    dfe <- plyr::rbind.fill(dfe, pe_table)
  }

  df <- merge_with_input(new_data = dfe, input = df, kpi = "eps_ex_nri")
  return(df)
}

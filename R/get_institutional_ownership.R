...
get_institutional_ownership <- function(df) {
  ...
  for(i in 1:rows) {
    ...
    ticker <- df$Symbol[i]
    url <- paste0("https://www.gurufocus.com/term/InstitutionalOwnership/", ticker, "/")
    ...
    if (any(dim(table) == FALSE)) {
      ic_table <- data.frame("Symbol" = ticker, "institutional_ownership_ttm" = cval)
    } else {
      colnames(table) <- gsub("-", "_", paste0("institutional_ownership_", table[1, ]))
      table <- table[-1, ]

      dfc <- data.frame(cval)
      colnames(dfc) <- "institutional_ownership_ttm"
  ...
  df <- merge_with_input(new_data = dfe, input = df, kpi = "institutional_ownership")
  return(df)
}

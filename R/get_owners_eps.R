...
get_owners_eps <- function(df) {
  ...
  for(i in 1:rows) {
    ...
    ticker <- df$Symbol[i]
    url <- paste0("https://www.gurufocus.com/term/Owner_Earnings/", ticker, "/")

    xpath <- '//*[@id="target_def_historical_data"]/div[2]/div/table'
    table <- get_html_table(url, xpath = xpath, name = "owners_eps_20")
    ...
    colnames(dfc) <- "owners_eps_TTM"

    pe_table <- cbind(ticker, table, dfc)
    colnames(pe_table)[1] <- "Symbol"

    if (check_table_topicality(table, ttmpe))
    { pe_table <- data.frame("Symbol" = ticker, "owners_eps_TTM" = NA) }

    dfe <- plyr::rbind.fill(dfe, pe_table)
  }

  df <- merge_with_input(new_data = dfe, input = df, kpi = "owners_eps")
  return(df)
}

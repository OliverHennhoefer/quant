...
get_ev <- function(df) {
  ...
  for(i in 1:rows) {
    ...
    ticker <- df$Symbol[i]
    url <- paste0("https://www.gurufocus.com/term/ev/", ticker, "/")

    xpath <- '//*[@id="target_def_historical_data"]/div[2]/div/table'
    table <- get_html_table(url, xpath = xpath, name = "ev_20")
    ...
    { pe_table <- data.frame("Symbol" = ticker, "ev_TTM" = NA) }

    dfe <- plyr::rbind.fill(dfe, pe_table)
  }

  df <- merge_with_input(new_data = dfe, input = df, kpi = "ev")
  return(df)
}

...
get_fin_distress_prob <- function(df) {
  check_sanity(df)

  df$fin_distress_prob <- NA
  rows <- nrow(df)
  pbar <- utils::txtProgressBar(0, rows, style = 3)
  pfix <- "fin_distress_prob"
  ...
  for(i in 1:rows) {
    ...
    ticker <- df$Symbol[i]
    url <- paste0("https://www.gurufocus.com/term/PFD/", ticker, "/")

    xpath <- '//*[@id="def_body_detail_height"]/font[1]'
    fin_dis <- get_html_text(url, xpath = xpath)

    extr_str <- gsub("\\s*\\([^\\)]+\\)", "\\1", fin_dis)
    df$fin_distress_prob[i] <- as.numeric(gsub("[^0-9.-]+", "\\1", extr_str))
  }

...
#'
get_price_to_free_cashflow <- function(df) {
  check_sanity(df)
  ...
  for(i in 1:rows) {
    ...
    ticker <- df$Symbol[i]
    url <- paste0("https://www.gurufocus.com/term/pfcf/",
                  ticker, "/")

    xpath = '//*[@id="target_def_historical_data"]/div[2]/div/table'
    table <- get_html_table(url, xpath = xpath, name = "Price2FCF_20")
    ...
    dfc <- data.frame(cval)
    colnames(dfc) <- "Price2FCF_TTM"
    ic_table <- cbind(ticker, table, dfc)
    colnames(ic_table)[1] <- "Symbol"

    if (check_table_topicality(table, ttmpe))
    { ic_table <- data.frame("Symbol" = ticker, "Price2FCF_TTM" = NA) }

    dfe <- plyr::rbind.fill(dfe, ic_table)
  }

  df <- merge_with_input(new_data = dfe, input = df, kpi = "Price2FCF")
  return(df)
}

...
#'
get_fin_strength_ttm <- function(df) {
  check_sanity(df)
  ...
  for(i in 1:rows) {
    ...
    ticker <- df$Symbol[i]
    url <- paste0("https://www.gurufocus.com/term/rank_balancesheet/", ticker, "/")

    xpath <- '//*[@id="target_def_historical_data"]/div[2]/div/table'
    table <- get_html_table(url, xpath = xpath, name = "fin_strength_20")
    ...
    colnames(dfc) <- "fin_strength_TTM"

    pe_table <- cbind(ticker, table, dfc)
    colnames(pe_table)[1] <- "Symbol"

    if (any(colnames(pe_table) == "remove")) pe_table <- pe_table[, !(names(pe_table) %in% "remove")]

    if (check_table_topicality(table, ttmpe))
    { pe_table <- data.frame("Symbol" = ticker, "fin_strength_TTM" = NA) }

    dfe <- plyr::rbind.fill(dfe, pe_table)
  }

  df <- merge_with_input(new_data = dfe, input = df, kpi = "fin_strength")
  return(df)
}
